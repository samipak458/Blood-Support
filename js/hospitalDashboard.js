...
onAuthStateChanged(auth, (donor) => {
    if (donor) {
       ...
       console.log("Your session is expired or you are log out");
        location = "hospitalDashboard.html";
    }
});
...
onAuthStateChanged(auth, (donor) => {
    ...
    if (donor) {
        ...
        const docRef = doc(fs, 'organizations', donor.uid);
        ...
        onSnapshot(docRef, (def) => {
            ...
            displayname.innerHTML = "Organization: " + def.data().Name;

            //Donor Email
            donoremail.innerHTML = def.data().Email;

            // Donor ID
            id.innerText = "Organization ID: " + def.data().ID;
...
for (let i = 0; i < Request.length; i++) {
    Request[i].addEventListener('click', () => {
          ...
          onAuthStateChanged(auth, (userCredential) => {
            if (userCredential) {
                updateDoc(doc(fs, 'organizations', userCredential.uid), {
